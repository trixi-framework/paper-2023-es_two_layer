# This file contains the equation folder for the two-layer shallow water equations
...
"""
struct ShallowWaterTwoLayerEquations2D{RealT<:Real} <: Trixi.AbstractShallowWaterEquations{2, 7}
...
Trixi.have_nonconservative_terms(::ShallowWaterTwoLayerEquations2D) = Trixi.True()
Trixi.varnames(::typeof(cons2cons), ::ShallowWaterTwoLayerEquations2D) = (
...
# conditions
Trixi.varnames(::typeof(cons2prim), ::ShallowWaterTwoLayerEquations2D) = (
...
"""
function Trixi.initial_condition_convergence_test(x, t, equations::ShallowWaterTwoLayerEquations2D)
...
"""
@inline function Trixi.source_terms_convergence_test(u, x, t, equations::ShallowWaterTwoLayerEquations2D)
...
@inline function Trixi.flux(u, normal_direction::AbstractVector, 
                        equations::ShallowWaterTwoLayerEquations2D)
    h_upper, h_lower = Trixi.waterheight(u, equations)
    v1_upper, v2_upper, v1_lower, v2_lower = velocity(u, equations)
...
@inline function flux_nonconservative_ersing_etal(u_ll, u_rr, 
    ...
    h_upper_ll, h_lower_ll = Trixi.waterheight(u_ll, equations)
    h_upper_rr, h_lower_rr = Trixi.waterheight(u_rr, equations)
    b_rr = u_rr[7]
    b_ll = u_ll[7]
...
@inline function Trixi.max_abs_speed_naive(u_ll, u_rr, 
    ...
    return max(abs(v_m_ll), abs(v_m_rr)) + max(c_ll, c_rr) * Trixi.norm(normal_direction)
end
...
@inline function Trixi.max_abs_speeds(u, equations::ShallowWaterTwoLayerEquations2D)
    h_upper, h_v1_upper, h_v2_upper, h_lower, h_v1_lower, h_v2_lower, _ = u
...
@inline function Trixi.velocity(u, equations::ShallowWaterTwoLayerEquations2D)
    h_upper, h_v1_upper, h_v2_upper, h_lower, h_v1_lower, h_v2_lower, _ = u
...
@inline function Trixi.cons2prim(u, equations::ShallowWaterTwoLayerEquations2D)
...
@inline function Trixi.cons2entropy(u, equations::ShallowWaterTwoLayerEquations2D)
    h_upper, _, _, h_lower, _, _, b = u
...
@inline function Trixi.prim2cons(prim, equations::ShallowWaterTwoLayerEquations2D)
...
@inline function Trixi.waterheight(u, equations::ShallowWaterTwoLayerEquations2D)
    return SVector(u[1], u[4])
end


# Entropy function for the shallow water equations is the total energy
@inline Trixi.entropy(cons, equations::ShallowWaterTwoLayerEquations2D) = energy_total(cons, equations)


# Calculate total energy for a conservative state `cons`
@inline function Trixi.energy_total(cons, equations::ShallowWaterTwoLayerEquations2D)
...
@inline function Trixi.energy_kinetic(u, equations::ShallowWaterTwoLayerEquations2D)
    ...
    return (0.5 * equations.rho_upper * h_v1_upper^2 / h_upper +
            ...
            0.5 * equaTrixi.l(cons, equations) - energy_kinetic(cons, equations))
end


# Calculate the error for the "lake-at-rest" test case where H = h_upper+h_lower+b should
# be a constant value over time
@inline function Trixi.lake_at_rest_error(u, equations::ShallowWaterTwoLayerEquations2D)
    h_upper, _, _, h_lower, _, _, b = u
